...
public static class ProjectValidator
{
    public static IReadOnlyList<ValidationIssue> Validate(ProjectIr project, int pointerSizeBytes)
    {
        ...
        foreach (var t in project.Types)
        {
            ...
            if (t.DeclaredSize == null && t.Size <= 0)
                ...
                {
                    IsError = false,
                    Message = $"Type '{t.Name}': size could not be inferred (add explicit size or resolve field types)"
                });
            ...
            if (t.ParentName != null)
            {
                if (!typeMap.TryGetValue(t.ParentName, out var parent))
                {
                    ...
                    {
                        IsError = false,
                        Message = $"Type '{t.Name}': parent '{t.ParentName}' not found — parent-boundary check indeterminate"
                    });
                }
                else if (parent.Size <= 0)
                {
                    ...
                    {
                        IsError = false,
                        Message = $"Type '{t.Name}': parent '{t.ParentName}' has unknown size — boundary indeterminate"
                    });
                }
                else
                {
    ...
    private static void CollectUnresolved(TypeExpr expr, string typeName, string fieldName,
        IReadOnlyDictionary<string, TypeDecl> map, List<ValidationIssue> issues)
    {
        switch (expr)
        {
